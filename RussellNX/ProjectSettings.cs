...
namespace RussellNX
{
    public partial class ProjectSettings : Form
    {
        ...
        private string SwitchOptionsDir { get; set; }

        private string SplashErrString { get; set; } = "Error when reading file.";
        private bool UseDbgOut { get; set; }

        private string RuntimeFolder { get; set; }
        ...
        public void ApplyLocalisation()
        {
            ...
            if (ci == "ru-RU")
            {
                ...
                SplashErrString = "Не могу открыть файл сплэша.";
            }
        }

        public ProjectSettings(string projpath, string runtimefolder) : this()
        {
        ...
        public void ParseFile(string path)
        {
            ...
            SwitchOptionsDir = Path.GetDirectoryName(path);
            ...
            if (jsettings.resourceVersion is null || jsettings.tags is null) IsNuBeta = false;

            if (IsNuBeta)
            {
                checkUseSplash.Checked = jsettings.option_switch_use_splash;
                buttonViewSplash.Tag = jsettings.option_switch_splash_screen;
                UseDbgOut = jsettings.option_switch_allow_debug_output;
            }
            else
            {
                // no please.
        ...
        private void SaveSettingsBtn_Click(object sender, EventArgs e)
        {
            ...
            if (IsNuBeta)
            {
                ...
                jsettings.Add("option_switch_allow_debug_output", UseDbgOut); // in RNX we always allow it :p
        ...
        {
            using (Process p = Process.Start(UnescapeGMMacros((string)buttonViewSplash.Tag)))
            {
                // ...
            }
        }
        ...
        private string UnescapeGMMacros(string _In)
        {
            ...
            _Out = _Out.Replace("${options_dir}", Path.GetDirectoryName(SwitchOptionsDir));
            _Out = _Out.Replace("${base_options_dir}", Path.Combine(RuntimeFolder, "BaseProject", "options"));
        ...
        private void buttonChangeSplash_Click(object sender, EventArgs e)
        {
            if (openSplashDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                if (stream != null)
                {
                    ...
                    {
                        stream.Dispose();
                        File.Copy(openSplashDialog.FileName, Path.Combine(SwitchOptionsDir, "splash.png"), true);
                        buttonViewSplash.Tag = "${options_dir}/switch/splash.png";
                        MessageBox.Show("OK. Resave.", "OK.", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
                    catch { }
                }

                MessageBox.Show(SplashErrString, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
            }
        }
    }
}
